...
La funci\'on de distribuci\'on acumulativa, tienen derivada en todas partes, excepto en $x = 0$. y $x= 1$. Las funciones de distribuci\'on de variables aleatorias  deben ser \texttt{absolutamente  continuas}, esto es, que son continuas y que sus derivadas existen salvo por un conjunto finito de puntos.
 \end{ejemplo}
 
 
 \begin{ejemplo}
 \normalfont Consideramos una variable aleatoria $X$, con una funci\'on de distribuci\'on definida como:
