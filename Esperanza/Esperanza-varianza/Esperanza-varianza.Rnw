...
\[
\mathbb{P}(X = 2^k) = \biggl(\frac{1}{2}\biggr),\ \ k = 1, 2, 3,\dots
\]

Por tanto,

\[
\mathbb{E}(X) = \sum_{k =1}^{\infty}2^k \biggl(\frac{1}{2}\biggr) = \sum_{k =1}^{\infty}1 = 1 + 1 +1 +\cdots = \infty.
\]
...
$\displaystyle\int_{-\infty}^{-t}f_X(x) dx = F_X(-t)$ y $\displaystyle\int_{t}^{\infty}f_X(x) dx = \mathbb{P}(X > t)=  1 -F(t)$. En general, para alguna variable aleatoria $X$,
