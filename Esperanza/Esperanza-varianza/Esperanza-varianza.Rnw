...
\[
C_X^2 = \frac{\text{Var}(X)}{(\mathbb{E}(X))^2}.
\]
...
\begin{align*}
\text{Var}(X) &= \sum_{i =2}^{12}(x_i - 7)^2p_i \\
...
Por tanto $\text{Var}(X) = 1075- (32.5)^2 = 18.75$ y as\'i $\delta_X = \sqrt{\text{Var}(X)} = 4.33$.
...
\begin{align*}
\text{Var}(M) = \bigints_{0}^{\infty}(x - \mathbb{E}(M))^2e^{-x}dx  = \bigints_{0}^{\infty}(x -1)^2e^{-x}dx \\
