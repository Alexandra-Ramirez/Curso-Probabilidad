...
\item Se lanzan dos dados. Sea $E$ el evento en  que la suma de las salidad es impar y $F$ el evento de al menos uno $1$. Interpretar los eventos $E\cap F, E^c \cap F$ y $E^cF^c$.
