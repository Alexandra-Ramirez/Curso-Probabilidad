...
\end{itemize}
\item Sean $X$ y $Y$ variables aleatorias de Poisson y $T =  X +Y$. Suponiendo que $X$ e $Y$ no son independientes y de hecho $X = Y$. ?`Es verdad que $T \sim \text{Poisson}(\lambda)$?.
...
\item Encuentra el n\'umero esperado de pares de personas, $A$ y $B$, de modo que $A$ elija el nombre de $B$ y $B$ elija el nombre de $A$ (donde $A = B$ y el orden no importa).
